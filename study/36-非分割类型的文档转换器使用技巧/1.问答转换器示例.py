...
llm = OllamaLLM(model="qwen2.5:1.5b", base_url="http://localhost:11434")

# 3.输出内容
# for qa in transformer_documents[0].metadata.get("questions_and_answers"):
#     print("问答数据:", qa)
qa_prompt = PromptTemplate.from_template("""请将以下文档内容转换为问答对格式。
...
2. 每个知识点生成一个问答对
3. 问题要具体，答案要准确
4. 输出 JSON 数组格式：[{{"question": "...", "answer": "..."}}]

输出：""")
for doc in documents:
    response = llm.invoke(qa_prompt.format(content=doc.page_content))
